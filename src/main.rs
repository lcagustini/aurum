...
struct Text<'ttf, 'a> {
    ...
    rendered: Option<Vec<Option<sdl2::render::Texture<'a>>>>,
    raw: Vec<String>,
}
impl<'ttf, 'a> Text<'ttf, 'a> {
    ...
    fn render_text(&mut self, texture_creator: &'a sdl2::render::TextureCreator<sdl2::video::WindowContext>) {
        let mut rendered: Vec<Option<sdl2::render::Texture>> = Vec::new();

        for i in 0..self.raw.len() {
            let with_line_number = format!["{:n$} {}", i+1, self.raw[i as usize], n = number_of_digits(self.raw.len())];
            let surface = match self.font.render(&with_line_number).blended(Color::RGBA(255, 255, 255, 255)) {
                Ok(s) => s,
                Err(_) => {
                    rendered.push(None);
                    continue;
...
fn main() {
    ...
    'running: loop {
        ...
        for i in cursor.screen_y..(text.raw.len() as u32) {
            let rendered = match text.rendered {
                Some(ref r) => r,
                None => break,
            };
            let texture = match rendered[i as usize] {
                Some(ref t) => t,
                None => continue,
            };
